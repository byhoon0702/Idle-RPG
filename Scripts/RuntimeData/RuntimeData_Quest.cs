...
namespace RuntimeData
{
	...
	public class QuestInfo : BaseInfo
	{
		...
		public void OnGetReward(bool displayReward, bool showToast = false)
		{
			...
			{
				IdleNumber rewardCount = count / GoalCount;
				...
				for (int i = 0; i < rewardInfos.Count; i++)
				{
					var reward = rewardInfos[i].Clone();
					reward.Multiply(rewardCount);
					rewards.Add(reward);
				}

				PlatformManager.UserDB.AddRewards(rewards, displayReward, showToast);
