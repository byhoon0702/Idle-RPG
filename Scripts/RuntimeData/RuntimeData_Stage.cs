...
namespace RuntimeData
{
	...
	public class StageInfo
	{
		...
		public float SpawnInterval => stageData.enemyInfo.spawnInterval;
		#endregion
		...
		public IdleNumber UnitAttackPower(bool isBoss)
		{
			...
			float stageweight = PlatformManager.ConfigMeta.STAGE_WEIGHT;
			float levelweight = PlatformManager.ConfigMeta.LEVEL_WEIGHT;
			float bossweight = PlatformManager.ConfigMeta.BOSS_ATK_WEIGHT;
			float atkWeight = PlatformManager.ConfigMeta.UNIT_STATE_ATTACK_POWER_WEIGHT;

			float monsterAtkWeight = stageData.monsterStats.atkWeight; ;
			IdleNumber defaultValue = (IdleNumber)stageData.monsterStats.attackPower;

			if (stageListData.useOverrideStat)
			{
				defaultValue = (IdleNumber)stageListData.overrideMonsterStat.attackPower;
				monsterAtkWeight = stageListData.overrideMonsterStat.atkWeight;
			}

			if (monsterAtkWeight == 0)
			{
				monsterAtkWeight = 1f;
			}
			...
			return (isBoss ? bossresult : result) * monsterAtkWeight;
		}
		...
		public IdleNumber UnitHP(bool isBoss)
		{
			...
			float stageweight = PlatformManager.ConfigMeta.STAGE_WEIGHT;
			float levelweight = PlatformManager.ConfigMeta.LEVEL_WEIGHT;
			float bossweight = PlatformManager.ConfigMeta.BOSS_HP_WEIGHT;
			float hpWeight = PlatformManager.ConfigMeta.UNIT_STATE_HP_WEIGHT;

			float monsterHpWeight = stageData.monsterStats.hpWeight;
			IdleNumber defaultValue = (IdleNumber)stageData.monsterStats.hp;

			if (stageListData.useOverrideStat)
			{
				defaultValue = (IdleNumber)stageListData.overrideMonsterStat.hp;
				monsterHpWeight = stageListData.overrideMonsterStat.hpWeight;
			}

			if (monsterHpWeight == 0)
			{
				monsterHpWeight = 1f;
			}
			...
			return (isBoss ? bossresult : result) * monsterHpWeight;
		}
