...
namespace RuntimeData
{
	...
	public class TrainingInfo : StatInfo
	{
		...
		{
			cost = ((IdleNumber)rawData.basicCost + (Mathf.Max(0, _level - 1) * rawData.basicCostInc)) * (1 + (rawData.basicCostWeight * _level));
			return cost;
		}
		public IdleNumber GetCost(int level)
		{
			cost = ((IdleNumber)rawData.basicCost + (Mathf.Max(0, level - 1) * rawData.basicCostInc)) * (1 + (rawData.basicCostWeight * level));
			return cost;
		}
