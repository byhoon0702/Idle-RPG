...
public class SkillSlot : DataSlot
{
	...
	public void Trigger(Unit caster, System.Action callback = null)
	{
		if (caster.IsAlive() == false)
		{
			return;
		}
		var skill = ScriptableObject.Instantiate(item.itemObject);
		skill.Trigger(caster, item, callback);
	}
...
public class SkillContainer : BaseContainer
{
	...
	{
		SkillContainer temp = new SkillContainer();
        temp  = JsonUtility.FromJson< SkillContainer>(json);
