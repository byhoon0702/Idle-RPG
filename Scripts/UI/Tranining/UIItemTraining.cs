...
public class UIItemTraining : MonoBehaviour
{
			...
			case LevelUpCount.MAX:
				{
					...
					while (totalCost <= currency.Value && ((trainingInfo.rawData.maxLevel > 0 && level < trainingInfo.rawData.maxLevel) || trainingInfo.rawData.maxLevel == 0))
					{
	...
	private void AbilityLevelUp(bool isRepeat)
	{
		...
		int count = levelUpCount;
		animator.SetTrigger("Selected");
		currency.Pay(_cost);
		trainingInfo.ClickLevelup(count);
