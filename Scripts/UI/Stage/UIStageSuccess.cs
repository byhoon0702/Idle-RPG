...
public class UIStageSuccess : UIStageResult
{
	...
	{
		bossTimer = isOn;
		_StageManager.it.continueBossChallenge = isOn;
		SwitchButton(isOn);
	...
	public override void Show(StageRule _rule)
	{
		...
		currentStage = _StageManager.it.CurrentStage;
		...
		if (rule is StageNormal)
		{
			...
			onClickLeft = () => { rule.End(); _StageManager.it.playBossStage = false; _StageManager.it.continueBossChallenge = false; };
			onClickRight = () => { rule.End(); _StageManager.it.playBossStage = true; };
			toggleBossContinue.SetIsOnWithoutNotify(_StageManager.it.continueBossChallenge);
