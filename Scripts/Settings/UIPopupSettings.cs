...
public class UIPopupSettings : UIBase
{
	...
	[Header("COUPON")]
	[SerializeField] private GameObject objCoupon;
	[SerializeField] private TMP_InputField couponCodeInput;
	...
	public async void OnClickInputCoupon()
	{
		PlatformManager.Instance.ShowLoadingRotate(true);
		var coupon = await PlatformManager.Instance.Coupon.CallRedeemCoupon(couponCodeInput.text);
		PlatformManager.Instance.ShowLoadingRotate(false);
		if (coupon != null)
		{
			bool parse = System.Enum.TryParse<RewardCategory>(coupon.reward.type, out RewardCategory category);
			if (parse)
			{
				RuntimeData.RewardInfo info = new RuntimeData.RewardInfo(coupon.reward.id, category, (IdleNumber)coupon.reward.amount);

				var rewardList = RewardUtil.ReArrangReward(new List<RuntimeData.RewardInfo>() { info });
