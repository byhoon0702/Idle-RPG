...
public class StageRule : ScriptableObject
{
	...
	public virtual void Begin()
	{
		...
		_StageManager.it.currentKillCount = 0;
		_StageManager.it.bossKillCount = 0;
		_StageManager.it.cumulativeDamage = (IdleNumber)0;
		_StageManager.it.usePhase = false;
	...
	public virtual void AddReward()
	{
		...
		_StageManager.it.CurrentStage.SetStageReward((IdleNumber)_StageManager.it.CurrentStage.StageNumber - 1);
		List<RuntimeData.RewardInfo> rewardList = new List<RuntimeData.RewardInfo>();

		var list = _StageManager.it.CurrentStage.GetStageRewardList();
	...
	public bool CheckEnd()
	{
		if (CheckLose())
		{
			isWin = false;
			isEnd = true;
			_StageManager.it.OnStageEnd(false);
			return true;
		}
		if (CheckWin())
		{
