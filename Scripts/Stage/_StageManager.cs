...
public class _StageManager : MonoBehaviour
{
	private static _StageManager instance;
	public static _StageManager it => instance;
	...
	public void ShowBoss()
	{
		...
		Transform[] bossSpawnPoses = map.bossSpawnPos;
		Vector3 pos = bossSpawnPoses != null && bossSpawnPoses.Length > 0 ? bossSpawnPoses[UnityEngine.Random.Range(0, bossSpawnPoses.Length - 1)].position : new Vector3(2, 0, 0);
	...
	public void OnStageEnd(bool isWin)
	{
		...
		if (isWin)
		{
			...
			{
				PlatformManager.Instance.LeaderBoard.AddStageScore();
			}
			if (CurrentStage.StageType == StageType.Dungeon || CurrentStage.StageType == StageType.Guardian)
			{
				...
				if (data.tid == 5010000002)
				{
					PlatformManager.Instance.LeaderBoard.AddScore(LeaderBoard.k_TreasureId, currentKillCount);
				}
	...
	public void SetOfflineKill()
	{
		...
		if (killCountForOffline >= UserDB.killLimit)
		{
			...
			int killCount = (int)(1000 / ts.TotalMinutes);
			if (PlatformManager.UserDB.userInfoContainer.userInfo.KillPerMinutes < killCount)
			{
				PlatformManager.UserDB.userInfoContainer.userInfo.KillPerMinutes = killCount;
			}
		}
