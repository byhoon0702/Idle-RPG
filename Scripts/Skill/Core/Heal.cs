...
public class Heal : SkillCore
{
	public override bool Trigger(Unit _caster, RuntimeData.SkillInfo _info, System.Action _callback)
	{
		...
		{
			if (allowedUnitState.HasFlag(_caster.GetState()) == false)
			{
				return false;
			}
		}
	...
	protected override void OnAffectTarget(Unit caster, Unit target, AffectedInfo hitInfo, float knockbackPower, Vector3 knockbackDir, SkillInfo skillInfo)
	{
		HealTarget(caster, target, skillInfo, hitInfo);
	}

	private void HealTarget(Unit caster, Unit target, RuntimeData.SkillInfo skillInfo, AffectedInfo affectedInfo)
	{
