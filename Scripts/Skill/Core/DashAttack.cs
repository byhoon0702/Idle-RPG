	...
	protected override IEnumerator Activation(Unit caster, RuntimeData.SkillInfo info, AffectedInfo hitInfo)
	{
		...
		while (attackCount < info.TargetCount)
		{
			...
			if (distance > 1)
			{
				...
				////Vector3 pos = StageManager.it.MoveRestrict(caster.position);
				//caster.GetComponent<Rigidbody2D>().velocity = caster.GetComponent<Rigidbody2D>().position + dirVec * data.speed * 2;
				yield return null;
			}
			else
			{
